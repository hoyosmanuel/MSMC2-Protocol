...
.. note::
   ...
   This is an additional key step in the workflow. It is independent from the heterozygosity estimation described in the precious step. Both analyses can be performed in parallel however, running both workflows simultaneously may be computationally/mentally demanding particularly when working with a large number of samples and therefore it is not a recommended standard approach. Again, this implementation is presented for transparency, and users may identify more efficient and streamlined alternatives.
